...
func New(userUUID string) (*Cart, error) {
	...
	if err != nil {
		cartError := entity.Error("cart").AddValidationError(err)
...
func (c *Cart) AddProductInCart(item product.IProduct) error {
	...
	err := validate.New[any]("item", item, validate.IsNil())
	if err != nil {
		return err
	}
	...
	for _, appliedCoupon := range c.appliedCoupons {
		discountApplicable := appliedCoupon.IsProductValid(productCart)
		...
		if discountApplicable {
			...
			productCart.AddCoupon(appliedCoupon)
		}
	}

	// Com os devidos descontos aplicados
	// Criamos o produto no map e atualizamos o subtotal, desconto aplicado e total final
...
func (c *Cart) RemoveItemInCart(productUUID string) error {
	...
	product, exists := c.products[c.cartUUID.String()]
	if !exists {
		return nil
	}
...
func (c *Cart) AddCoupon(coupon coupon.ICoupon) error {
	...
	err := validate.New[any]("coupon", coupon, validate.IsNil())
	if err != nil {
		return err
	}
	...
	for _, item := range c.products {
		...
		isApplicable := coupon.IsProductValid(item)
		if isApplicable {
...
func (c *Cart) RemoveCoupon(couponUUID string) error {
	...
	// verificar se o cupom está presente no carrinho
	var indice int
...
func (c *Cart) calculateDiscount() {
	for _, item := range c.products {
		c.total.discounts = item.TotalDiscountApplied()
		c.total.totalFinal = item.FinalTotal()
		c.total.subtotal = item.Subtotal()
	}
}
